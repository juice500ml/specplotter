...
Examples:
  ...
  specplotter audio.wav -o output.png --sample-rate 22050
    ...
    parser.add_argument(
        '--zcr',
        action='store_true',
        help='Show zero crossing rate plot'
    )
    
    parser.add_argument(
        '--total-energy',
        action='store_true',
        help='Show total energy plot'
    )
    
    parser.add_argument(
        '--lowfreq-energy',
        action='store_true',
        help='Show low frequency energy plot'
    )
    
    parser.add_argument(
        '--waveform',
        action='store_true',
        help='Show waveform plot'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=60,
        help='Dynamic range in dB (default: 60)'
    )
    ...
    if not input_path.suffix.lower() in ['.wav', '.wave']:
        print(f"Warning: Input file may not be a WAV file: {args.input_file}", file=sys.stderr)
    ...
    try:
        ...
        print(f"  Components: spectrogram", end="")
        if show_zcr:
            print(" + zcr", end="")
