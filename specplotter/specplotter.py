...
class SpecPlotter(object):
    ...
    def __init__(
        ...
        self.window_stride = window_stride if window_stride is not None else default_window_stride
        self.n_fft = n_fft
        self.window = window
    ...
    def _compute_zcr(self, signal: np.ndarray) -> np.ndarray:
        ...
        g = scipy.signal.windows.gaussian(self.zcr_smoothing_size, std=self.zcr_smoothing_std)
        g = g / g.sum()
    ...
    def _compute_lowfreq_energy(self, power_spec: np.ndarray) -> np.ndarray:
        ...
        g = scipy.signal.windows.gaussian(self.zcr_smoothing_size, std=self.zcr_smoothing_std)
        g = g / g.sum()
        f0 = int(np.round((self.lowfreq_min / self.assumed_rate * 0.5) * power_spec.shape[0]))
        f1 = int(np.round((self.lowfreq_max / self.assumed_rate * 0.5) * power_spec.shape[0]))
    ...
    def plot_zcr(
        self, zcr: np.ndarray, ax: Optional[Axes] = None, **kwargs
    ) -> Axes:
    ...
    def plot_total_energy(
        ...
        color = kwargs.get('color', 'gray')
        ax.fill_between(
            np.arange(len(total_energy)), total_energy, y2=total_energy.min(), color=color
        )
        ax.margins(0, 0)
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    def plot_lowfreq_energy(
        ...
        color = kwargs.get('color', 'gray')
        ax.fill_between(
            np.arange(len(lowfreq_energy)), lowfreq_energy, 
            y2=lowfreq_energy.min(), color=color
        )
        ax.margins(0, 0)
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    def _plot_spectrogram_on_axis(
        self, spectrogram: np.ndarray, signal_length: int, 
        ...
        extent = [
            0, signal_length / self.assumed_rate, 0,
    ...
    def plot(
        self, signal: np.ndarray, ax: Optional[Union[Axes, List[Axes]]] = None,
        show_zcr: bool = False, show_total_energy: bool = False,
        show_lowfreq_energy: bool = False, show_waveform: bool = False,
        ...
        **kwargs
        ...
        if ax is None:
            ...
            if n_plots == 1:
                ...
                fig = plt.figure(figsize=(n_sec * self.inches_per_sec, self.fig_height))
                gs = fig.add_gridspec(nrows=n_plots, ncols=1, height_ratios=height_ratios, hspace=0.05)
        ...
        for plot_name in plot_order:
            if plot_name == 'zcr':
                ...
                self.plot_total_energy(features['total_energy'], axes['total_energy'], **kwargs)
                ...
                self._plot_spectrogram_on_axis(
                    features['spectrogram'], signal.shape[0], axes['spectrogram'], **kwargs
                )
            elif plot_name == 'waveform':
                self.plot_waveform(features['processed_signal'], axes['waveform'], **kwargs)
    ...
    def plot_spectrogram(
        self, signal: np.ndarray, ax: Optional[Axes] = None,
        outfile: Optional[str] = None, **kwargs
        ...
        **kwargs
        ...
        """
        fig, axes = self.plot(
            signal, ax=ax, show_zcr=False, show_total_energy=False,
            show_lowfreq_energy=False, show_waveform=False, outfile=outfile, **kwargs
        )
